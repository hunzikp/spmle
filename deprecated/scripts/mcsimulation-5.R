...
fit_spprobit <- function(data, method = c('bayes', 'spmle', 'ris', 'gmm', 'naiveprobit')) {
  ...
  res_names <- c('time', beta_names, 'rho_hat')  #'mem',
  res_ls <- as.list(c(perf[3],  beta, rho)) # mem ,
...
param_tb <- expand.grid(N = c(2^8),
                        ...
                        beta0 = 0,
                        beta1 = 1,
                        seed = c(1:2), # Replications per config # number of MCs
                        method = c('gmm', 'spmle', 'naiveprobit', 'ris', 'bayes'), #  'spmle', 'naiveprobit', 'ris',  'gmm' , ' bayes'
...
results_tb %>%
  ...
  summarise(time_mean=mean(time, na.rm=T),
            ...
            # mem_mean=mean(mem, na.rm=T),
            # mem_max=max(mem, na.rm=T),
            # mem_sd=sd(mem, na.rm=T),
