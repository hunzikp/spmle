...
fit_spprobit <- function(data, method = c('bayes', 'spmle', 'ris', 'gmm', 'naiveprobit')) {
  ...
  res_names <- c('time', 'mem', beta_names, 'rho_hat')
  res_ls <- as.list(c(perf[3], mem , beta, rho))
...
param_tb <- expand.grid(N = c(2^8),
                        ...
                        beta0 = 0,
                        beta1 = 1,
                        seed = c(1:30), # Replications per config # number of MCs
                        method = c('spmle', 'naiveprobit', 'ris'), #    ,' bayes', 'gmm'
